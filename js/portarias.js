...
// Estilos de ajuste (Atos Normativos): negrito/sublinhado sempre em linha,
// formulário de edição em 100% da largura e sem botão "Editar" por bloco.
(function () {
  if (document.getElementById("portarias-ajustes-css")) return;
  const st = document.createElement("style");
  st.id = "portarias-ajustes-css";
  st.textContent = `
    .fl-valor, .port-section-text { display:block; }
    .fl-valor b, .fl-valor strong, .fl-valor u, .fl-valor i,
    .port-section-text b, .port-section-text strong, .port-section-text u, .port-section-text i {
      display:inline !important; float:none !important; width:auto !important; flex:none !important;
    }
    #port-detail-pane .admin-inline-form { max-width:100% !important; width:100%; box-sizing:border-box; }
    #port-detail-pane .admin-inline-form input[type="text"],
    #port-detail-pane .admin-inline-form select,
    #port-detail-pane .admin-inline-form textarea { width:100% !important; max-width:100% !important; box-sizing:border-box; }
  `;
...
async function salvarFormularioEstadoCompleto(uf) {
  ...
  if (fichaInput && fichaInput.files && fichaInput.files[0]) {
    ...
    if (arq.size > 3 * 1024 * 1024) {
      alert("O arquivo da Ficha Técnica passa de 3 MB. Escolha um arquivo menor.");
      return;
    }
    try {
  ...
  const resultado = await resyncDataBlock("portarias", PORTARIAS_DATA);
  Object.keys(PORTARIAS_DATA).forEach(k => {
    const ft = PORTARIAS_DATA[k] && PORTARIAS_DATA[k].fichaTecnica;
    if (ft && ft.dataUrl) {
      FICHA_CACHE[k] = ft.dataUrl;
      PORTARIAS_DATA[k].fichaTecnica = { nome: ft.nome, tipo: ft.tipo, tamanho: ft.tamanho, mantida: true };
    }
  });
  selectEstado(uf);
  const header = document.querySelector("#port-detail-pane .port-detail-header");
...
const NOMES_BLOCO_PORTARIA = {
  fichaTecnica: "Ficha Técnica Institucional",
...
function fichaTecnicaHtml(f, uf) {
  return `<a href="#" onclick="abrirFichaTecnica('${uf}');return false;" style="color:inherit;font:inherit;letter-spacing:inherit;text-transform:inherit;text-decoration:underline;cursor:pointer;" title="Baixar arquivo">Ficha Técnica Institucional</a>`;
}

// Baixa o arquivo. O conteúdo só é buscado no banco no momento do clique (deixa o carregamento leve).
async function abrirFichaTecnica(uf) {
  const meta = PORTARIAS_DATA[uf] && PORTARIAS_DATA[uf].fichaTecnica;
  if (!meta) return;
  let dataUrl = meta.dataUrl || FICHA_CACHE[uf];
  try {
    if (!dataUrl) {
      const resp = await fetch(`${SUPABASE_URL}/rest/v1/portarias_estados?select=ficha_tecnica&uf=eq.${encodeURIComponent(uf)}`, {
        headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }
      });
      if (!resp.ok) throw new Error("status " + resp.status);
      const linhas = await resp.json();
      const f = linhas[0] && linhas[0].ficha_tecnica;
      dataUrl = f && f.dataUrl;
      if (dataUrl) FICHA_CACHE[uf] = dataUrl;
    }
    if (!dataUrl) { alert("Arquivo da Ficha Técnica não encontrado."); return; }
    ...
    a.href = url; a.download = meta.nome || "ficha-tecnica";
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 60000);
  } catch (e) {
    alert("Não foi possível baixar a Ficha Técnica: " + e.message);
  }
}
