...
async function salvarFormularioEstadoCompleto(uf) {
  ...
  if (fichaInput && fichaInput.files && fichaInput.files[0]) {
    ...
    try {
      ...
      dados.fichaTecnica = { nome: arq.name, tipo: arq.type || "", dataUrl };
    } catch (e) {
...
// ---- Ficha Técnica Institucional (arquivo anexado) ----
function fichaTecnicaHtml(f, uf) {
  const ehImagem = /^data:image\//.test(f.dataUrl);
  const icone = /pdf/i.test(f.nome) ? "📕" : /docx?$/i.test(f.nome) ? "📘" : /xlsx?m?$/i.test(f.nome) ? "📗" : "🖼️";
  const preview = ehImagem ? `<img src="${f.dataUrl}" alt="Ficha Técnica Institucional" style="max-width:100%;border:1px solid var(--cinza-borda);border-radius:8px;margin-bottom:8px;display:block;">` : "";
  return `${preview}<a href="#" onclick="abrirFichaTecnica('${uf}');return false;" style="color:var(--roxo);font-weight:600;text-decoration:none;">${icone} ${escapeHtmlRegra(f.nome)}</a>`;
}
